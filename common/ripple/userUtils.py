...
def PPBoard(userID, relax):
    result = glob.db.fetch(
        "SELECT ppboard FROM {rx}_stats WHERE id = {userid}".format(rx='rx' if relax else 'users', userid=userID))
    return result['ppboard']


def setPPBoard(userID, rx):
    glob.db.execute(
        "UPDATE {rx}_stats SET ppboard = 1 WHERE id = {userid}".format(rx='rx' if rx else 'users', userid=userID))


def setScoreBoard(userID, rx):
    glob.db.execute(
        "UPDATE {rx}_stats SET ppboard = 0 WHERE id = {userid}".format(rx='rx' if rx else 'users', userid=userID))


def noPPLimit(userID, relax):
    result = glob.db.fetch(
        "SELECT unrestricted_pp FROM {rx}_stats WHERE id = {userid}".format(rx='rx' if relax else 'users',
                                                                            userid=userID))
    return result['unrestricted_pp']


def whitelistUserPPLimit(userID, rx):
    glob.db.execute("UPDATE {rx}_stats SET unrestricted_pp = 1 WHERE id = {userid}".format(rx='rx' if rx else 'users',
                                                                                           userid=userID))
...
def isLocked(userID):
    ...
    if result is not None:
        return (
                (result["privileges"] & privileges.USER_PUBLIC > 0) and (
...
def verifyUser(userID, hashes):
    ...
    for i in hashes[2:5]:
        if i == "":
            log.warning("Invalid hash set ({}) for user {} while verifying the account".format(str(hashes), userID),
                        "bunk")
            return False
    ...
    if hashes[2] == "b4ec3c4334a0249dae95c284ec5983df" or hashes[4] == "ffae06fb022871fe9beb58b005c5e21d":
        ...
        log.info(
            "{user} ({userID}) ha triggerato Sannino:\n**Full data:** {hashes}\n**Usual wine mac address hash:** b4ec3c4334a0249dae95c284ec5983df\n**Usual wine disk id:** ffae06fb022871fe9beb58b005c5e21d".format(
                user=username, userID=userID, hashes=hashes), "bunker")
    ...
    if match:
        ...
        appendNotes(userID,
                    "{}'s multiaccount ({}), found HWID match while verifying account ({})".format(originalUsername,
                                                                                                   originalUserID,
                                                                                                   hashes[2:5]))
