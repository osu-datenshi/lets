			...
			if beatmapInfo is not None and beatmapInfo != False and s.passed:
				...
				if newScoreboard.personalBestRank == 1 and s.completed == 3 and not restricted:
					...
					if UsingRelax:
						...
						embed.add_embed_field(name='Accuracy: {}%'.format(s.accuracy * 100), value='Combo: {}/{}'.format(s.maxCombo, beatmapInfo.maxCombo))
						...
						embed.set_image(url='https://assets.ppy.sh/beatmaps/{}/covers/cover.jpg'.format(beatmapInfo.beatmapSetID))
						webhook.add_embed(embed)
